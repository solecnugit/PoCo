...
export class NearService {
   ...
   * 获取广播者的所有任务
   */
  async getBroadcasterTasks(): Promise<TaskData[]> {
    try {
      if (!this.isSignedIn() || !this.contract) {
        throw new Error("请先登录NEAR钱包");
      }

      const result = await this.contract.get_broadcaster_tasks({
        broadcaster_id: this.accountId,
      });

      return result;
    } catch (error) {
      console.error("获取广播者任务失败:", error);
      throw error;
    }
  }
