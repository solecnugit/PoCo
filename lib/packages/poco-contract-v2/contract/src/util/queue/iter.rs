        ...
        T: BorshSerialize + BorshDeserialize,
{
    pub(super) fn new(queue: &'a CircularQueue<T>) -> Self {
        Self { queue, range: Range { start: queue.offset, end: queue.offset + queue.len() } }
    }
        ...
        T: BorshSerialize + BorshDeserialize,
{
    ...
    fn nth(&mut self, n: usize) -> Option<Self::Item> {
        ...
        Some(self.queue.get(idx).unwrap_or_else(|| near_sdk::env::panic_str(ERR_INDEX_OUT_OF_BOUNDS)))
    }
}
        ...
        T: BorshSerialize + BorshDeserialize,
{
    pub(crate) fn new(queue: &'a mut CircularQueue<T>) -> Self {
        ...
        Self { queue, range: Range { start: offset, end } }
    }
        ...
        T: BorshSerialize + BorshDeserialize,
{
    fn get_mut<'b>(&'b mut self, at: u64) -> Option<&'a mut T> {
        self.queue.get_mut(at).map(|value| {
        ...
        T: BorshSerialize + BorshDeserialize,
{
    ...
    fn nth(&mut self, n: usize) -> Option<Self::Item> {
        let idx = self.range.nth(n)?;
        Some(self.get_mut(idx).unwrap_or_else(|| near_sdk::env::panic_str(ERR_INDEX_OUT_OF_BOUNDS)))
    }
}
