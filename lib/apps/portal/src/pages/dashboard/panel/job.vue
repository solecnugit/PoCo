...
<template>
    <div class="bg-white/20 backdrop-blur-md p-4 border border-gray-200 rounded-md shadow-md flex flex-col">
        <div class="flex justify-between">
            ...
            <div>
                <button @click="poco.postJob" type="button"
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 rounded-md table-fixed">
                <thead class="text-xs text-white uppercase border-b-2">
                    <tr class="table-fixed">
                        <th scope="col" class="h-16 px-4 w-16">
                            JobId
                        </th>
                        <th scope="col" class="h-16 px-4">
                            From
                        </th>
                        <th scope="col" class="h-16 px-2">
                            Action & Progress
                        </th>
                    </tr>
                </thead>
                <tbody>
                    <tr class="border-b table-fixed text-white cursor-pointer" v-for="item in jobs"
                        :key="item.jobId.toString()" :class="{' text-gray-200': item.status === 'done'}">
                        <th scope="row"
                        ...
                        <td class="h-16 px-4 whitespace-nowrap text-ellipsis overflow-hidden">
                            {{item.owner}}
                        </td>
                        <td class="h-16 px-2 gap-1">
                            <div class="flex items-center gap-2">
                                <button v-if="item.status === 'pending'" :disabled="item.status !== 'pending'"
                                ...
                                <button type="button" v-if="item.buffer" @click="download(item.jobId, item.buffer!)"
                                ...
                                <button type="button" @click="setJobFile(item.jobId)" v-if="item.isOwn"
